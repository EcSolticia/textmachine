...
pub enum GenError {
    ...
    LogicError(String),
    #[error("[textmachine-gen-presentation-error] {0}")]
    GenPresentationError(String),
...
impl GenPandocOutput {
    pub fn get_path(&self) -> Result<String, GenError> {
        if let Some(path_str) = self.path.to_str() {
            return Ok(String::from(path_str))
        } else {
            return Err(
                GenError::GenPresentationError(
                    String::from("cannot present pandoc output path due to invalid unicode characters")
                )
            )
        }
    }
...
impl GenOutput {
    pub fn present_pandoc_outputs(&self) -> Result<(), GenError> {
        println!("\n");
        println!("[textmachine-pandoc-output] The following presents the pages generated by pandoc");
        
        for po in &self.pandoc_outputs {
            println!("[textmachine-pandoc-output] {}", po.get_path()?);
        }
